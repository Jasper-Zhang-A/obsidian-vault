...
# 投影到 YZ-平面：一个“困难模式”的探索 (例7)

#calculus #flux #surface-integral #common-mistakes

---

## 问题回顾与目标

我们仍然求解同一个问题：计算矢量场 $\mathbf{F}$ 穿过抛物柱面 $y=x^2$ 的向外通量。但这一次，我们挑战自己，坚持将曲面投影到 **yz-平面**上。

> [!warning] 前方高能预警
> 这种方法是可行的，但比投影到 xz-平面要复杂得多，也更容易出错。这个练习的重点在于理解**为什么**它更难，以及如何在复杂的变量替换中保持正确。

## 梯度法 (YZ-平面投影) 步骤

前两步和我们之前讨论的梯度法完全一样。

-   **曲面函数:** $f(x,y,z) = x^2 - y = 0$。
-   **梯度 (法向量):** $\nabla f = 2x\mathbf{i} - \mathbf{j}$。
-   **所需方向:** "向外"方向，我们选择 $+\nabla f$。

### **第三步 (新变化): 确定 YZ-平面的投影公式**

当我们将曲面 $S$ 投影到 yz-平面时，对应的面积元 $dA = dy\,dz$。通量公式中的 $\mathbf{n}\,d\sigma$ 需要用如下方式转换：
$$ \mathbf{n}\,d\sigma = \frac{\pm \nabla f}{|\nabla f \cdot \mathbf{i}|} dA $$
这里的 $\mathbf{i}$ 是 yz-平面自身的法向量。
...
这是最容易出错的一步。因为我们的积分变量是 `y` 和 `z` ($dA = dy\,dz$)，所以积分号内的**所有表达式**都必须用 `y` 和 `z` 来表示。

1.  **确定投影区域 R:**
    -   由题目可知 $0 \le z \le 4$。
    -   同时 $0 \le x \le 1$。因为 $y=x^2$，所以 $y$ 的范围是 $0^2 \le y \le 1^2$，即 $0 \le y \le 1$。
    -   所以，在 yz-平面上的投影区域 R 是一个矩形：$0 \le y \le 1, 0 \le z \le 4$。

2.  **计算公式的各个部分:**
    -   分子 $(\mathbf{F} \cdot \nabla f)$: 我们已经算过是 $2x^3z - x$。
    -   分母 $|\nabla f \cdot \mathbf{i}|$:
        $$ |\nabla f \cdot \mathbf{i}| = |(2x\mathbf{i} - \mathbf{j}) \cdot \mathbf{i}| = |2x| $$
        因为在我们的定义域里 $0 \le x \le 1$，所以 $|2x|=2x$。

3.  **进行变量替换 (统一为 y 和 z):**
    我们的被积函数现在是：
    $$ \frac{2x^3z - x}{2x} = x^2z - \frac{1}{2} $$
    现在，我们必须用 $y$ 和 $z$ 来表示它。从曲面方程 $y=x^2$ 我们得到 $x = \sqrt{y}$ (因为 $x \ge 0$)。将它代入上式：
    $$ (\sqrt{y})^2 z - \frac{1}{2} = yz - \frac{1}{2} $$
    **检查一下：** 这个表达式里只剩下 `y` 和 `z` 了，说明变量替换成功。你的错误很可能就发生在这里，比如忘记替换某个 `x`。

### **第五步：计算最终的积分**

现在我们可以建立一个只包含 `y` 和 `z` 的、干净的二重积分了。

$$ \text{通量} = \int_0^4 \int_0^1 \left(yz - \frac{1}{2}\right) dy\,dz $$
1.  **先对 y 积分:**
    $$ \int_0^1 \left(yz - \frac{1}{2}\right) dy = \left[ \frac{y^2}{2}z - \frac{y}{2} \right]_0^1 = \left(\frac{1}{2}z - \frac{1}{2}\right) - (0) = \frac{z-1}{2} $$
2.  **再对 z 积分:**
    $$ \int_0^4 \frac{z-1}{2} dz = \frac{1}{2} \left[ \frac{z^2}{2} - z \right]_0^4 = \frac{1}{2} \left( (\frac{16}{2}-4) - 0 \right) = \frac{1}{2}(8-4) = 2 $$

## 结论与反思

> [!success]
> 最终结果为 **2**，和投影到 xz-平面的结果完全一样！这证明了只要每一步都正确，无论选择哪个投影平面，答案都是唯一的。

> [!tip] 为什么投影到 YZ-平面更困难？
>
> -   **需要变量代换:** 投影到 xz-平面时，我们的积分变量是 `x` 和 `z`，而曲面方程 $y=x^2$ 和被积函数可以很自然地用 `x` 和 `z` 表示，几乎不需要额外的代换。
> -   **投影到 yz-平面时，积分变量是 `y` 和 `z`。我们必须执行一个额外的步骤：用 $x=\sqrt{y}$ 把所有 `x` 都替换掉。** 这一步不仅增加了计算量，也引入了新的出错点（例如忘记替换，或者开方时出错）。
>
> **选择投影平面的黄金法则：** 尽量选择能让曲面方程 $z=f(x,y)$ 或 $y=g(x,z)$ 的**自变量**作为积分变量的平面。这样可以最大程度地简化计算，避免复杂的变量代换。