...
当一个物理量 $N$ 不是通过直接测量得到，而是由一个或多个独立测量的物理量 $x, y, z, ...$ 通过函数关系 $N=f(x,y,z,...)$ 计算得出时，它的不确定度需要通过不确定度传播规律来计算。

### 1. 误差传递的基本思想 (基于全微分)

[cite_start]我们可以通过函数的全微分来分析各个直接测量量的误差是如何传递给最终结果的 [cite: 1623, 1631]。
- **一般形式**：
  [cite_start]$$ dN=\frac{\partial f}{\partial x}dx+\frac{\partial f}{\partial y}dy+\frac{\partial f}{\partial z}dz+\cdot\cdot\cdot $$ [cite: 1625]
- [cite_start]**对数形式**：当函数是乘除、幂次形式时，先取对数再微分会更方便 [cite: 1630]。
  [cite_start]$$ \frac{dN}{N}=\frac{\partial \ln f}{\partial x}dx+\frac{\partial \ln f}{\partial y}dy+\frac{\partial \ln f}{\partial z}dz+\cdot\cdot\cdot $$ [cite: 1629]

### 2. 不确定度传播公式 (Transfer Expressions for Uncertainty)

[cite_start]由于各个独立测量量的误差是随机且不相关的，它们对总不确定度的贡献应按方和根（即“勾股定理”的形式）进行合成 [cite: 1635]。

- **通用不确定度传播公式**:
  [cite_start]$$ \sigma_{N}=\sqrt{(\frac{\partial f}{\partial x})^{2}\sigma_{x}^{2}+(\frac{\partial f}{\partial y})^{2}\sigma_{y}^{2}+(\frac{\partial f}{\partial z})^{2}\sigma_{z}^{2}+\cdot\cdot\cdot} $$ [cite: 1637]
  此公式为最常用的核心公式。

- **相对不确定度传播公式**:
  [cite_start]对于乘除、幂次形式的函数，使用相对不确定度计算更简便 [cite: 1640, 1641]。
  [cite_start]$$ \frac{\sigma_{N}}{N}=\sqrt{(\frac{\partial \ln f}{\partial x})^{2}\sigma_{x}^{2}+(\frac{\partial \ln f}{\partial y})^{2}\sigma_{y}^{2}+(\frac{\partial \ln f}{\partial z})^{2}\sigma_{z}^{2}+\cdot\cdot\cdot} $$ [cite: 1641]

- **公式的应用**：
  - [cite_start]计算间接测量量 $N$ 的合成不确定度 $\sigma_N$ [cite: 1642]。
  - [cite_start]分析各个分量的不确定度（$\sigma_x, \sigma_y, ...$）对总不确定度 $\sigma_N$ 的贡献大小，从而帮助我们进行合理的误差分配和选择合适的测量仪器 [cite: 1643, 1644]。

### 3. 简化计算 - 小误差忽略原则

[cite_start]在进行不确定度合成的计算时，可以遵循小项忽略原则以简化计算 [cite: 1645]。
- [cite_start]**原则**：在计算方和根时，如果某一项（例如 $(\frac{\partial f}{\partial y})^{2}\sigma_{y}^{2}$）远小于其中的最大项，则可以忽略此项 [cite: 1647]。
- [cite_start]**判断依据**：通常，如果一个不确定度分量（如 $|\frac{\partial f}{\partial y}\sigma_{y}|$）**小于**最大分量的 **1/3**，那么它的平方项将小于最大项平方的 1/9 (约 1/10)，此时就可以在求和中将其忽略 [cite: 1647]。