...
#calculus #vector-calculus #greens-theorem #stokes-theorem #divergence-theorem #parametrization
...
[[Gauss's Divergence Theorem]]
### 3.1 定理的完整形式
$$ \oiint_{S} \vec{F} \cdot d\vec{S} = \iiint_V (\nabla \cdot \vec{F}) \,dV $$

### 3.2 公式详解

#### **左侧 (曲面积分 / 通量)**
-   **矢量形式:**
    $$ \oiint_{S} \vec{F} \cdot d\vec{S} \quad \text{或} \quad \oiint_{S} \vec{F} \cdot \vec{n} \,dS $$
-   **分量形式:**
    $$ \oiint_{S} P\,dy\,dz + Q\,dz\,dx + R\,dx\,dy $$
